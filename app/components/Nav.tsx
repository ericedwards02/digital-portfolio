...
export default function Nav({ setActivePost }: NavProps) {
  return (
    <ul className="list-none p-0 m-0 min-w-[175px]">
      <li className="text-lg font-bold">portfolio</li>
      {posts.map((post, index) => (
        <li
          key={post.id}
          onClick={() => setActivePost(post)}
          className="text-lg text-gray-500 transition-all duration-250 cursor-pointer mb-2.5 hover:text-xl
          hover:text-[var(--subheaders)] hover:font-bold"
        >
